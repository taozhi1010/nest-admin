...
export class UploadService {
	...
	async singleFileUpload(file: Express.Multer.File) {
		...
		if (fileSize > this.config.get('app.file.maxSize')) {
			throw ResultData.fail(500, `文件大小不能超过${this.config.get('app.file.maxSize')}MB`);
		}
		let res;
		if (this.isLocal) {
	...
	async chunkFileUpload(file: Express.Multer.File, body: ChunkFileDto) {
		...
		const chunckFilePath = path.join(
			chunckDirPath,
			`${body.uploadId}${body.fileName}@${body.index}`
		);
	...
	async checkChunkFile(body) {
		...
		if (!fs.existsSync(chunckFilePath)) {
			throw ResultData.fail(500, '文件不存在');
		} else {
			return ResultData.ok();
		}
	}
	...
	async saveFileLocal(file: Express.Multer.File) {
		...
		// 文件服务完整路径
		const fileName = path
			.join(this.config.get('app.file.serveRoot'), relativeFilePath)
			.replace(/\\/g, '/');
	...
	getNewFileName(originalname: string): string {
		...
		newFileNameArr[newFileNameArr.length - 1] =
			`${newFileNameArr[newFileNameArr.length - 1]}_${new Date().getTime()}`;
	...
	async getChunkUploadResult(uploadId: string) {
		...
		if (data) {
			return ResultData.ok({
				data,
				message: data.status === '0' ? '上传成功' : '上传中',
			});
		} else {
			throw ResultData.fail(500, '文件不存在');
		}
	 ...
	 * @returns
	 */
	async singleFileUploadExtended(
		...
		// 使用适配器模式处理不同存储类型
		const storage = this.storageAdapter?.getStorage(storageType) || null;
		let res;

		if (storage) {
			res = await storage.saveFile(targetPath, newFileName, file);
		} else {
			// 回退到原有逻辑 - 使用现有的上传方法
			res = await this.singleFileUpload(file);
		}
		...
		await this.sysUploadEntityRep.save({
			...
			size: file.size,
			status: '0',
			createBy: user?.user?.username || user?.username || 'system',
