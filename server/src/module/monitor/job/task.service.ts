...
export class TaskService implements OnModuleInit {
	...
	private async initializeTasks() {
		...
		for (const { classOrigin, methodName, taskName } of tasks) {
			try {
				// 获取或创建服务实例
				...
				this.taskMap.set(taskName, method);
				this.logger.log(`注册任务: ${taskName}`);
			} catch (error) {
				this.logger.error(`注册任务失败 ${taskName}: ${error.message}`);
			}
		}
	}
